...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById('map'), {
      ...
      //Texas
      center:{lat: park_json.lat, lng: park_json.lon},
      scrollwheel: false,
      zoom: 12
    });
...
var ViewModel = function() {
    ...
    self.park = ko.observable(new Park(park_json));
    ...
    //park_json.trails.forEach(function(trail){
    //  self.trailList( new Trail(trail) );
    //});
...
// API CALLS //////////////////////////////////////////
    ...
    self.getForecast = function( park ) {
        ...
        var position = park.position;

        // FORECAST
        var xmlhttp = new XMLHttpRequest();
        //query OpenWeatherMap by geolocation
        var url = "http://api.openweathermap.org/data/2.5/forecast/daily?lat="
          +position.lat()+"&lon="+position.lng()+
          "&mode=json&units=imperial&cnt=7&APPID=1088269cadd02d84dba9b274fc7bc097";
    ...
    self.getDistance = function( result ) {
        ...
        var destinationA = self.park().position;
